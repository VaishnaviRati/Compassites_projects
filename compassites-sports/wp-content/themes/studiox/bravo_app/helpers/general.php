...
if(!function_exists('bravo_custom_field_types'))
{
    function bravo_custom_field_types()
    {
        $types= array(
            array(
                'value'=>'text',
                'label'=>esc_html__('Text',"studiox")
            ),
            array(
                'value'=>'textarea',
                'label'=>esc_html__('Textarea',"studiox")
            ),
            array(
                'value'=>'date',
                'label'=>esc_html__('Date',"studiox")
            ),
            array(
                'value'=>'color',
                'label'=>esc_html__('Colorpicker',"studiox")
            ),
            array(
                'value'=>'on-off',
                'label'=>esc_html__('On/Off',"studiox")
            ),
        );
...
if ( ! function_exists( 'bravo_entry_meta' ) ) :
    ...
    function bravo_entry_meta() {
       echo sprintf('<a href="%s" class="readmore" role="button">'.esc_html__('Read More',"studiox").'</a>',get_permalink());
        ?>

            <div class="article-meta-wrapper">
                <span class="article-meta"><?php esc_html_e('By',"studiox")?> <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php the_author_meta( 'display_name' ); ?>"><?php the_author_meta( 'display_name' ); ?></a></span>
                <span class="article-meta"><?php echo get_the_category_list(', ')?></span>
                <span class="article-meta"><?php
                    $view=bravo_get_post_view();
                    if($view>1){
                        printf(esc_html__('%d Views',"studiox"),esc_attr($view));
                    }else{
                        printf(esc_html__('%d View',"studiox"),esc_attr($view));
                    }
    ...
    function bravo_comment_nav()
    {
        ...
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ...
            <nav class="navigation comment-navigation" role="navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', "studiox" ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link(esc_html__( 'Older Comments', "studiox" ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', $prev_link );
                    endif;

                    if ( $next_link = get_next_comments_link(esc_html__( 'Newer Comments', "studiox" ) ) ) :
...
if(!function_exists('bravo_get_sidebar'))
{
    function bravo_get_sidebar()
    {
        ...
        $default= apply_filters('bravo_get_sidebar',$default);

		$all_sidebar=wp_get_sidebars_widgets();

		if(empty($all_sidebar[$default['id']]))
		{
			$default['position']='no';
		}
    ...
    function bravo_breadcrumb()
    {
        if (!is_home()) {
            ...
            }elseif(is_single() or is_page())
            {
                ...
                echo "<li class='active'>".esc_html__('404 Page',"studiox")."</li>";
            }
            echo "</ul>";
        }else{
            ...
            echo '">';
            echo esc_html__('Home',"studiox");
            echo "</a></li><li class='active'><a>".esc_html__('Blog',"studiox")."</a></li>  ";
            echo "</ol>";
        }
    }
}
...
if(!function_exists('bravo_get_order_list'))
{
    ...
    {
        $default=array(
            'none'=>esc_html__('None',"studiox"),
            'ID'=>esc_html__('Post ID',"studiox"),
            'author'=>esc_html__('Author',"studiox"),
            'title'=>esc_html__('Post Title',"studiox"),
            'name'=>esc_html__('Post Name',"studiox"),
            'date'=>esc_html__('Post Date',"studiox"),
            'modified'=>esc_html__('Last Modified Date',"studiox"),
            'parent'=>esc_html__('Post Parent',"studiox"),
            'rand'=>esc_html__('Random',"studiox"),
            'comment_count'=>esc_html__('Comment Count',"studiox"),
        );
...
if(!function_exists('bravo_get_list_taxonomy_id'))
{
    function bravo_get_list_taxonomy_id($tax = 'category', $array = array())
    {
        ...
        $r[esc_html__('All Categories', "studiox")] = 0;
    ...
    if(!function_exists('bravo_get_list_menu')){
        function bravo_get_list_menu(){
            ...
            $r[0]['value']='';
            $r[0]['label']=esc_html__("-- Select --","studiox");
            if (!empty($allMenu)) {
...
if(!function_exists('bravo_vc_post_dropdown')){
    function  bravo_vc_post_dropdown($arg=array()){
        ...
        $return=array();
        $return[esc_html__('-- Select --',"studiox")]=false;
    ...
    if(!function_exists('bravo_paginate_links')){
        function bravo_paginate_links( $args = '' ,$key='paged') {
            ...
            $defaults = array(
                ...
                'show_all' => false,
                'prev_next' => true,
                'prev_text' =>'<i class="fa fa-long-arrow-left"></i> '.__("Newest Posts","studiox"),
                'next_text' =>'<i class="fa fa-long-arrow-right"></i> '.__("Older Posts","studiox"),
                'end_size' => 1,
                'mid_size' => 2,
                'type' => 'plain',
            ...
            if ( $args['prev_next'] && $current && 1 < $current ) :
                 ...
                 */
                $page_links[] = '<a class="morepost pull-left" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . do_shortcode($args['prev_text']) . '</a>';
            endif;
            /*for ( $n = 1; $n <= $total; $n++ ) :
                if ( $n == $current ) :
                    ...
                    elseif ( $dots && ! $args['show_all'] ) :
                        $page_links[] = '<li><span class="page-numbers dots">' .esc_html__( '&hellip;',"studiox" ) . '</span></li>';
                        $dots = false;
                    endif;
                endif;
            endfor;*/
            if ( $args['prev_next'] && $current && ( $current < $total || -1 == $total ) ) :
                ...
                $page_links[] = '<a class="morepost pull-right" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . do_shortcode($args['next_text']) . '</a>';
            endif;
            switch ( $args['type'] ) {
                case 'array' :
                    return $page_links;
